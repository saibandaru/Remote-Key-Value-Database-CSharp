...
namespace Project4Starter
{
  ...
  class Server
  {
        ...
        public static void notifyEventcoming(Sender sndr, String command, Message msg,string address)
        {           
            Message newmsg = new Message();
            MessageMaker mm = new MessageMaker();
            newmsg.fromUrl = msg.toUrl;
            newmsg.toUrl = Util.makeUrl("localhost", "9080");
            newmsg.content = mm.constructLog(address, command);//constructLog(string req, string command_t, string info)
            ...
            Action serviceAction = () => { Message msg = null; ulong count_no = 0,previous=0;          
                while (true) {
                    ...
                    try {
                        ...
                        notifyEventcoming(sndr, ms.getCommand(msg), msg, msg.fromUrl);
                    ...
                    if (!msg.content.Equals("notify"))
                    {
                        ...
                        MessageMaker mm = new MessageMaker();
                        notifyEventcoming(sndr, mm.parseRequestMessageForString(msg, "RESULT"), msg, msg.toUrl);
                    }
                }
            };
